...
func doRestoreNew(ssc *SscClient, jobs []openapi.ApiJob, Share string, FileName string, Directory string, verbose bool) error {
...
func executeSearchNew(ssc *SscClient, args *Arguments) error {
	...
	for fileNameIndex := range fileNames {
		...
		ret, err := doSearchNew(mySsc, fileName, exts, verbose)
		if err != nil {
			return fmt.Errorf("search objects for match %s failed %v\n", fileName, err)
		}
